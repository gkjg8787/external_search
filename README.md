...
- apiの種類
  - 検索
    - 指定したURL、または検索ワードから価格情報を取得する
  - カテゴリー一覧
    - 指定したサイトのカテゴリー一覧を取得する
  - ダウンロード設定の生成
    - gemini を使ったダウンロードオプションの自動生成
...
- gemini api を使用したスクレイピング。直接 URL を AI にスクレイピングしてもらうのではなくパーサを作成してもらいそれを活用する。URL のダウンロードは selenium or httpx を使用するがダウンロードができない場合もある。
...
| ----------------- | ---------------------------------------------------- | ---------- |
| cookie            | cookie の設定                                        | dict       |
| wait_css_selector | 指定したタグ(css セレクタ)が描写されるまで待つ設定   | dict       |
| page_wait_time    | wait_css_selector を指定しない場合の読み込み待ち時間 | 数値       |
...
| オプション名  | 説明                        | 設定する値   |
| ------------- | --------------------------- | ------------ |
| selector      | 描写を待つ css セレクタ     | CSS セレクタ |
| timeout       | タイムアウト秒              | 数値         |
| on_error      | エラー時の動作設定          | dict         |
| pre_wait_time | selector を待つ前の待機時間 | 数値         |

- on_error のオプション

| オプション名    | 説明                                                                                                                                           | 設定する値   |
| --------------- | ---------------------------------------------------------------------------------------------------------------------------------------------- | ------------ |
| action_type     | raise or retry                                                                                                                                 | 文字列       |
...
- response
  - AI によるパーサ生成のため取得できる情報は異なる結果になる場合がある。
...
### ダウンロード設定の自動生成
- gemini を使った検索オプション(options)の自動生成を行う。
  - 指定されたURLをダウンロードし、gemini apiへ問い合わせ、必要な設定の構築を行い返却する。
  - ダウンロードは複数回行う場合もある。
- API : `/downloadconfig/generate/` を使用する。

| パラメータ名   | 説明                | 設定する値 | デフォルト |
| -------------- | ------------------- | ---------- | ---------- |
| url            | ダウンロードオプションを作成したい 検索結果のURL | 有効な URL |             |
| search_keyword | URLで検索クエリとして使っている検索キーワード | 文字列      |             |
